...
echo [%time%] Exit code: !errorlevel!
powershell -NoProfile -Command "& {$c=Get-Content '!TEMP_DIR!\!BASE!_placement_raw.csv';$c[0]='Designator,Val,Package,\"Mid X\",\"Mid Y\",Rotation,Layer';Set-Content '!TEMP_DIR!\!BASE!_placement.[...]
...
powershell -NoProfile -Command "& {$csv=Import-Csv '!TEMP_DIR!\bom_raw.csv';$g=@{};foreach($r in $csv){$m=$r.MPN.Trim();if([string]::IsNullOrWhiteSpace($m)){$k=$r.Designator;$um=$r.MPN}else{$k=$m[...]
echo [%time%] Exit code: !errorlevel!
